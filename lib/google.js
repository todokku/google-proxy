const SEARCH_URL = 'https://www.google.com/search',
    COMPLETE_URL = 'https://www.google.com/complete/search',
    itemSel = '#rso > .g',
    linkSel = '.r > a',
    titleSel = '.r > a > h3',
    descSel = '.s span.st',
...
const parseResult = function(body){
    let parseItem =function($, elem){
        let $elem = $(elem);
...
module.exports = function(req, res){
    ...
    let search = function(){
        ...
        req.query.hl = req.query.hl || getLanguage();

        return proxy(req).then(parseResult).then(json =>{
            json.q = req.query.q;
            json.hl = req.query.hl;
            if(req.cookies['hl'] !== req.query.hl){
                res.cookie('hl', req.query.hl, { maxAge: 31536000000, httpOnly: true });
            }
            return json;
        });
    };
