var SEARCH_URL = 'https://www.google.com/search?q=%s&start=%s',
    ...
    imageListSel = 'img',
    imageScriptSelList = 'script:contains(\'_setImagesSrc\')', //主体
    imageScriptSelPatch = 'script:contains(\'google.ldi\')', //补丁
    relevantSel = '#brs p > a',
...
var parseResult = function(body){
    ...
    if(images.length){
        ...
        for (let i = 0; i < imageScriptList.length; i++) {
            injectPresetBase64Script += $(imageScriptList[i]).html()
        }
        itemList.push({
            title: images.prev().find("h3 a").text(),
            type: "image",
            link: imageLinks,
            script: initialScript + (injectPresetBase64Script || injectSetImgSrcFunc) + ldiScript.substring(loc.start.column, loc.end.column) + injectScript
        });
    }
